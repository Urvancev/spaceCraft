...
unsigned int Sun_Dat(double H[4],double Si[3],double Scck[3],unsigned short Sn) // модель солнечного датчика
{
    ...
    q[0][0] = H[0]*H[0] + H[1]*H[1] - H[2]*H[2] - H[3]*H[3];
    q[1][1] = H[0]*H[0] - H[1]*H[1] + H[2]*H[2] - H[3]*H[3];
    q[2][2] = H[0]*H[0] - H[1]*H[1] - H[2]*H[2] + H[3]*H[3];

    q[0][1] = 2 * (H[1]*H[2] + H[0]*H[3]);
    q[0][2] = 2 * (H[1]*H[3] - H[0]*H[2]);

    q[1][0] = 2 * (H[1]*H[2] - H[0]*H[3]);
    q[1][2] = 2 * (H[2]*H[3] + H[0]*H[1]);

    q[2][0] = 2 * (H[1]*H[3] + H[0]*H[2]);
    q[2][1] = 2 * (H[2]*H[3] - H[0]*H[1]);

/*
    q[0][1] = 2 * H[1] * H[2] - 2 * H[3] * H[0];
    q[0][2] = 2 * H[1] * H[3] + 2 * H[2] * H[0];

    q[1][0] = 2 * H[1] * H[2] + 2 * H[3] * H[0];
    q[1][2] = 2 * H[2] * H[3] - 2 * H[1] * H[0];

    q[2][0] = 2 * H[1] * H[3] - 2 * H[2] * H[0];
    q[2][1] = 2 * H[2] * H[3] + 2 * H[1] * H[0];

    q[0][0] = 1 - 2 * H[2]*H[2] - 2 * H[3]*H[3];
    q[1][1] = 1 - 2 * H[1]*H[1] - 2 * H[3]*H[3];
    q[2][2] = 1 - 2 * H[1]*H[1] - 2 * H[2]*H[2];
*/
    Scck[0] = q[0][0] * Si[0] + q[0][1] * Si[1] + q[0][2] * Si[2];
    Scck[1] = q[1][0] * Si[0] + q[1][1] * Si[1] + q[1][2] * Si[2];
    Scck[2] = q[2][0] * Si[0] + q[2][1] * Si[1] + q[2][2] * Si[2];
